...
<script>
    ...
    async function searchVehicle() {
        ...
        try {
            ...
            const response = await fetch('vehicle_check.php', { method: 'POST', body: formData });
            if (!response.ok) {
            ...
            if (data.found) {
                ...
                const uiClass = (vehicleStatus === 'INACTIVE' || vehicleStatus === 'N/A') ? 'danger' : 'success';
                showVehicleResult(uiClass, 'Hasil Pengecekan', null, data.certified_to, data.status);
            } else {
    ...
    function showVehicleResult(status, heading, errorMessage, certifiedTo, vehicleStatus) {
        ...
        if (errorMessage) {
            result.innerHTML =
                '<div class="vehicle-check-result-heading" style="text-align:center; margin-bottom:10px;">' + escapeHtml(heading) + '</div>' +
        ...
        <?php if ($showDetailMRPButton && count($mrpByAreaFormatted) > 0): ?>
            const initialData = {
                ...
                unitInfo: <?php echo json_encode(
                    (!empty($provider->finalCriteria->merk) && !empty($provider->finalCriteria->model) && !empty($provider->finalCriteria->type) && !empty($provider->finalCriteria->tahun))
                    ? $provider->finalCriteria->merk . ' ' . $provider->finalCriteria->model . ' ' . $provider->finalCriteria->type . ' ' . $provider->finalCriteria->tahun
                    : '',
